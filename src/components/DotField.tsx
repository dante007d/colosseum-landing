...
const DotField = memo(
  ({
    ...
    gradientTo = "rgba(212, 175, 55, 0.15)",
    glowColor = "#0a0a0a",
    className = "",
    ...rest
  }: DotFieldProps) => {
    ...
    const dotsRef = useRef<Record<string, number>[]>([]);
      ...
      function tick() {
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (p.sparkle) {
            ...
            if (hash % 100 < 5) {
              ctx!.moveTo(drawX + rad * 2.5, drawY);
              ctx!.arc(drawX, drawY, rad * 2.5, 0, TWO_PI);
            } else {
        ...
        if (p.sparkle) {
          ...
          for (let i = 0; i < len; i += 10) {
            const d = dots[i];
            const hash = ((i * 2654435761) ^ frameCount) >>> 0;
            if (hash % 1000 < 2) {
              ctx!.moveTo(d.sx + rad * 4, d.sy);
              ctx!.arc(d.sx, d.sy, rad * 4, 0, TWO_PI);
            }
          }
          ctx!.fill();
        }
